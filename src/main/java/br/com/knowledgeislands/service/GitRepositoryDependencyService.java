...
import br.com.knowledgeislands.model.entity.GitRepositoryDependency;
...
import br.com.knowledgeislands.utils.GitRepositoryUtils;
...
public class GitRepositoryDependencyService {
	...
	public List<GitRepositoryDependency> getDependenciesProjectVersion(String projectFullName) {
		...
		try {
			...
			String command = br.com.knowledgeislands.utils.KnowledgeIslandsUtils.commandGetDependencyRepo.replace("$TOKEN", token).replace("$OWNER", owner).replace("$PROJECT", name);
